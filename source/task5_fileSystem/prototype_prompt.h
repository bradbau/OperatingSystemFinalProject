...
// char prompt[] = "prototype > ";
char theme[] = "Here to control prototype's file system";
...
void print_version() {
  char load[] = "....................";
  char ch[20];
  printf("PROTOTYPE File System: version v1.2\n\n");
  printf("Copyright (C) 2018 prototype\n\n");
  for (int i = 0; i < 19; i++) {
...
void print_usage(void) {
  printf("Usage: ./prototypefs [options]\n");
  printf("Options:\n");
...
int Start_Shell(int argc, char **argv) {
  ...
  /* Print informations */
  printf("Welcome to prototype's interactive shell!\n");
  printf("%s\n", theme);
  printf("\n");
  ...
  if (!developer_cmd(argc, argv)) {
    if (!builtin_cmd(argv)) {
      if (!py_execute(argv[0], argc, argv))
        printf("prototypesh: command not found: %s\n", argv[0]);
      // else
